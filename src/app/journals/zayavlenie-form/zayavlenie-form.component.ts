...
export class ZayavlenieFormComponent implements OnInit {
      ...
      this.formaService.getByIin(id).subscribe({
        next: (data: any) => {
          if (data) {
            ...
            this.vidZayavleniya = data.vidZayavleniya     || 'Новое назначение';
        ...
        error: () => {
          // Нет заявления в z_doc — загружаем данные клиента из m_eg
  ...
  private _buildKartaRows(m: EmdMaketRecord) {
    ...
    m.peraschetHistory.forEach((p, i) => {
      this.kartaRows.push({
        num: this.kartaRows.length + 1, nomerResheniya: m.id + '-REC' + (i + 1),
        viplata: m.maketIdCbd || '-', dateNachala: m.maketDateNazn, dateOkon: p.dateOkon,
        tipNaznacheniya: 'REC - перерасчет', summa: p.summaAfter,
        fio: this.fio, sid: '1234567', payId: '87434123', pnptId: '987654', isRec: true,
      });
    });
  }
  ...
  private _currentToMaketRecord(): EmdMaketRecord {
    ...
    return {
      ...
      maketNomerSpravki: this.maketNomerSpravki, maketStranaInvalidnosti: this.maketStranaInvalidnosti,
      maketDateUstanov: this.maketDateUstanov, maketDateOkonDo: this.maketDateOkonDo,
      maketGruppaInvalid: this.maketGruppaInvalid, maketPrichinaInvalid: this.maketPrichinaInvalid,
      maketNaznSumma: this.maketNaznSumma, maketIdCbd: this.maketIdCbd, maketIdEmaket: this.maketIdEmaket,
  ...
  private _startPereschet() {
    ...
    this.isPereschet = true; this.peraschetStep = 'editing';
    this.activeTab = 'maket'; this.cdr.detectChanges();
  }
  ...
  private _cancelPereschet() {
    ...
    this._recRows = []; this._maketSnapshot = null; this.isPereschet = false;
    this.peraschetStep = 'idle'; this.reshenieNeedsSign = false;
    this.activeTab = 'zayavlenie'; this.cdr.detectChanges();
  }
  ...
  saveMaket() {
    ...
    const recRow: KartaRazmerovRow = {
      ...
      dateOkon: this.maketDateOkon, tipNaznacheniya: 'REC - перерасчет',
      summa: Number(this.maketNaznSumma), fio: this.fio,
      sid: '1234567', payId: '87434123', pnptId: '987654', isRec: true,
    };
  ...
  addIzhdivents() { this.izhdivency.push({ otnoshenie: 'Сын/дочь', iin: '', fio: '', dateBirth: '' }); }
  ...
  formatDate(event: Event, field: 'dateBirth' | 'dateObr' | 'datePriem') {
    ...
    input.value = value; this[field] = value;
  }
